...
const WITHDRAW_COMPLAINT_API_URL =
  "http://localhost:8088/api/complaints/withdrawComplaint";

const normalizeComplaint = (complaint = {}) => {
  const source = complaint || {};
  return {
    ...source,
    complaintId: source.complaintId || source.complaint_id || source.id || "",
...
const getComplaintFromResponse = (response) => {
  ...
  return normalizeComplaint(response?.data || response?.complaint || response);
};
...
function CustomerServiceExecutive() {
  ...
  const filteredComplaints = complaints.filter((complaint) => {
    ...
    return [complaint.customerName, complaint.complaintId].some((value) =>
      String(value || "")
        .toLowerCase()
    ...
    <main className="role-main">
      ...
      >
        ...
        {activeTab === "raise" ? (
          <form className="complaint-form" onSubmit={handleSubmit}>
            ...
            <div className="form-field">
              ...
              <input
                ...
                name="complaintTitle"
                type="text"
                placeholder="Summarize the complaint"
                required
              />
            </div>
          ...
          <div className="complaint-list" role="tabpanel">
            {isLoadingComplaints ? (
              <p>Loading complaints...</p>
            ) : complaintListError ? (
              <p role="alert">{complaintListError}</p>
            ) : complaints.length === 0 ? (
              <div className="empty-list">
                <span className="empty-list-icon" aria-hidden="true">
                  +
                </span>
                <strong>No complaints raised yet</strong>
                <span>New complaints will appear here after submission.</span>
                <button type="button" onClick={() => setActiveTab("raise")}>
              ...
              <>
                <div className="complaint-search">
                  <label htmlFor="complaint-search-input">
                    Search complaints
                  </label>
                  <input
                    ...
                    placeholder="Search by customer name or complaint ID"
                  />
                </div>
                {filteredComplaints.length === 0 ? (
                  <div className="empty-list">
                    <strong>No complaints match your search.</strong>
                  </div>
                ) : (
                    ...
                    <table className="complaint-table">
                      ...
                      <tbody>
                        {filteredComplaints.map((complaint, index) => (
                          <tr
                            key={`${complaint.complaintId || complaint.id || complaint.customerEmail}-${index}`}
                          >
                            <td className="complaint-id table-cell">
                            ...
                            <td>
                              <button
                                ...
                                disabled={isWithdrawnComplaint(complaint)}
                              >
                                {isWithdrawnComplaint(complaint)
                                  ? "Withdrawn"
                                  : "Withdraw"}
                              </button>
                            </td>
        ...
        {withdrawComplaint && (
          <div
            ...
            role="presentation"
            onClick={() => !isWithdrawing && setWithdrawComplaint(null)}
          >
            <div
            ...
            >
              <h2 id="withdraw-confirmation-title">
              ...
              <div className="confirmation-actions">
                <button
                  ...
                  onClick={() => setWithdrawComplaint(null)}
